...
class VisualizadorDatosMeteorologicos:
    ...
    def agregar_punto_dato(self, temperatura, humedad, direccion_viento):
        tiempo_transcurrido = time.time() - self.tiempo_inicio
        self.tiempos_transcurridos.append(tiempo_transcurrido)
        self.temperaturas.append(temperatura)
        self.humedades.append(humedad)

        if direccion_viento in self.direcciones_labels:
            indice = self.direcciones_labels.index(direccion_viento)
            self.direcciones_contadores[indice] += 1

        self.actualizar_grafico()

def decodificar_datos(datos_codificados):
    # Convertir los 3 bytes recibidos a un entero de 24 bits
    payload = int.from_bytes(datos_codificados, byteorder='big')
    
    # Extraer temperatura (14 bits)
    temperatura = (payload >> 10) & (2**14 - 1)
    temperatura = (temperatura / (2**14 - 1)) * 110  # Escalar al rango original de 0-110 °C

    # Extraer humedad (7 bits)
    humedad = (payload >> 3) & (2**7 - 1)

    # Extraer dirección del viento (3 bits)
    direccion_viento_codificada = payload & 0b111
    direcciones_viento = ['N', 'NE', 'E', 'SE', 'S', 'SO', 'O', 'NO']
    direccion_viento = direcciones_viento[direccion_viento_codificada]
...
def consumir_datos_meteorologicos():
    ...
    try:
        while True:
            ...
            temperatura, humedad, direccion_viento = decodificar_datos(mensaje.value())
            print(f"Datos decodificados - Temperatura: {temperatura:.2f}°C, Humedad: {humedad}%, Dirección del Viento: {direccion_viento}")
            
            # Actualizar el visualizador con los datos decodificados
            visualizador.agregar_punto_dato(temperatura, humedad, direccion_viento)
